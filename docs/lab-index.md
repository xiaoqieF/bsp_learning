...
| 07 | Virtio Block + 持久化 rootfs | virtio-blk、ext4 | 可执行 |
| 08 | U-Boot 启动链 | U-Boot、Image、DTB、initrd | 可选，需要外部 U-Boot |
| 09 | PCI BAR + MMIO + IRQ | QEMU EDU PCI | 可执行 |
...
- Lab 07 使用 QEMU virtio-blk 和真正的 ext4 镜像，验证持久化 rootfs 启动。
- Lab 09 使用 QEMU EDU PCI 设备的 BAR 和中断；它不是设备树设备，用于对比 PCI 驱动模型。
